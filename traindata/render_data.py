...
def render_prt_ortho(out_path, folder_name, subject_name, rndr, rndr_uv, im_size, angl_step=4, n_light=1, pitch=[0]):
    # set path for ob=j, prt
    mesh_file = 'OBJ/'+ subject_name + '_100k.obj'
    #print('OBJ/'+subject_name + '_100k.obj')
    ...
    if not os.path.exists(text_file):
        text_file = os.path.join(folder_name, 'tex', subject_name + '_dif_2k.png')
        if not os.path.exists(text_file):
            print('ERROR: dif file does not exist!!', text_file)
            return             
    
    cam = Camera(width=im_size, height=im_size)
    cam.ortho_ratio = 0.2 * (1024 / im_size)
    cam.near = -100
    cam.far = 100
    cam.sanity_check()
    ...
    os.makedirs(os.path.join(out_path, 'NORM', subject_name),exist_ok=True)
    ...
    for p in pitch:
        for y in tqdm(range(0, 360, angl_step)):
            ...
            for j in range(n_light):
                ...
                dic = {'sh': None, 'ortho_ratio': cam.ortho_ratio, 'scale': y_scale, 'center': vmed, 'R': R}
                ...
                out_all_f = 255.0*cv2.cvtColor(out_all_f, cv2.COLOR_RGBA2BGR)
                for i in range(im_size):
                    for j_ in range(im_size):
                        if sum(out_all_f[i,j_]) ==0:
                            out_all_f[i,j_] = (255,255,255)
                out_depth = cv2.cvtColor(rndr.get_z_value(), cv2.COLOR_RGBA2BGR)
                out_norm = 255*cv2.cvtColor(rndr.get_color(1), cv2.COLOR_RGBA2BGR)
                for i in range(im_size):
                    for j_ in range(im_size):
                        if sum(out_norm[i,j_]) ==0:
                            out_norm[i,j_] = (255,128,128)
                        if y==180:
                            out_norm[i,j_] = 255 - out_norm[i,j_]

                np.save(os.path.join(out_path, 'PARAM', subject_name, '%d_%d_%02d.npy'%(y,p,j)),dic)
                cv2.imwrite(os.path.join(out_path, 'RENDER', subject_name, '%d_%d_%02d.jpg'%(y,p,j)),out_all_f)
                cv2.imwrite(os.path.join(out_path, 'MASK', subject_name, '%d_%d_%02d.png'%(y,p,j)),255.0*out_mask)
                cv2.imwrite(os.path.join(out_path, 'DEPTH', subject_name, '%d_%d_%02d.png'%(y,p,j)),255.0-255.0*out_depth)
                cv2.imwrite(os.path.join(out_path, 'NORM', subject_name, '%d_%d_%02d.png'%(y,p,j)),out_norm)
                '''
                #rndr_uv.set_sh(sh)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ip', '--inputpath', type=str, default='OBJ')
    ...
    if args.input != None:
        ...
        render_prt_ortho(args.out_dir, args.input, subject_name, rndr, rndr_uv, args.size, 180, 1, pitch=[0])
        
    else:
        filenames=os.listdir(args.inputpath)
        for file in filenames[:]:
            if file[-4:] == '_OBJ':
                args.input = os.path.join(args.inputpath, file)
